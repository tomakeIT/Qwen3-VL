...
# ==========================
# 5.2 生成阶段计时（prefill 与 decode）
# ==========================
class TimingLogitsProcessor(LogitsProcessor):
    """
    利用 LogitsProcessor 的调用时机，测量：
    - prefill 时间：从调用 generate 到第一次 processor 被调用之间
    - decode 时间：相邻两次 processor 调用之间的间隔，包含采样与一步前向
    """
    def __init__(self):
        super().__init__()
        self.t_start: Optional[float] = None
        self.first_call: bool = True
        self.prefill_time_s: Optional[float] = None
        self.decode_step_times_s: List[float] = []
    ...
    def finalize(self) -> None:
        # 在 generate 返回后补上最后一步 decode 的时间片（从最后一次调用到结束）
        if self._finalized:
            return
        if self._last_call_time is not None:
            end_t = self._now()
            self.decode_step_times_s.append(max(0.0, end_t - self._last_call_time))
        self._finalized = True

    def __call__(self, input_ids, scores):
        now = self._now()
        if self.first_call:
            # 第一次被调用：完成了 prefill 前向，进入首个解码步
            base = self.t_start if self.t_start is not None else now
            self.prefill_time_s = max(0.0, now - base)
            self.first_call = False
            self._last_call_time = now
        else:
            # 后续每次调用：记录上一个解码步到当前的间隔
            if self._last_call_time is not None:
                self.decode_step_times_s.append(max(0.0, now - self._last_call_time))
            self._last_call_time = now
        return scores

    @property
    def decode_total_time_s(self) -> float:
        return float(sum(self.decode_step_times_s)) if self.decode_step_times_s else 0.0
...
def run_gvl_qwen_for_trajectory(
    ...
    # 推理
    with torch.no_grad():
        timing_proc = TimingLogitsProcessor()
        timing_proc.start()
    ...
    # 统计与打印计时
    num_new_tokens = int(generated_ids_trimmed[0].shape[-1]) if len(generated_ids_trimmed) > 0 else 0
    prefill_ms = (timing_proc.prefill_time_s or 0.0) * 1000.0
    decode_ms = timing_proc.decode_total_time_s * 1000.0
    print(f"[计时] Prefill 用时: {prefill_ms:.2f} ms | Decode 总用时: {decode_ms:.2f} ms | 新生成token数: {num_new_tokens}")
